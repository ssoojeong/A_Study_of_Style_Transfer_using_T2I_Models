...
class CLIPLoss(torch.nn.Module):
    def __init__(self, device, lambda_direction=1., lambda_patch=0., lambda_global=0., lambda_manifold=0., lambda_texture=0., lambda_special=0., patch_loss_type='mae', direction_loss_type='cosine', clip_model='ViT-B/32'):
        ...
        ####special_cliploss
        self.lambda_special   = lambda_special
    ...
    #####special clip loss#####
    def special_patch_scores(self, src_img: torch.Tensor, trg_img: torch.Tensor, patch_centers, patch_size: int) -> torch.Tensor:
        
        # src_img = src_img.unsqueeze(0)
        # trg_img = trg_img.unsqueeze(0)
        
        src_patches        = self.generate_patches(src_img, patch_centers, patch_size)
        src_image_features = self.get_image_features(src_patches)
        
        trg_patches        = self.generate_patches(trg_img, patch_centers, patch_size)
        trg_image_features = self.get_image_features(trg_patches)

        #similarity = torch.nn.CosineSimilarity(dim=2)(src_image_features, trg_image_features)
        similarity = src_image_features @ trg_image_features.T
        org_similarity = src_image_features @ src_image_features.T
        #scores = torch.nn.CrossE
        scores = torch.nn.MSELoss()(similarity, org_similarity)
        return scores
    
    
    def special_clip_patch_similarity(self, src_img: torch.Tensor, target_img: torch.Tensor) -> torch.Tensor:
        patch_size = 196 #TODO remove magic number

        patch_centers = self.random_patch_centers(src_img.shape, 4, patch_size) #TODO remove magic number
   
        #scores    = 1.0 - self.special_patch_scores(src_img, target_img, patch_centers, patch_size)
        scores = self.special_patch_scores(src_img, target_img, patch_centers, patch_size)
    ...
    def forward(self, src_img: torch.Tensor, source_class: str, target_img: torch.Tensor, target_class: str, texture_image: torch.Tensor = None):
        ...
        #special_cliploss
        if self.lambda_special:
            special_cliploss = self.lambda_special * self.special_clip_patch_similarity(src_img, target_img)
            return clip_loss, special_cliploss
        #################
